...
func AsicInit(device *util.SerialDevice, initialized *int) {
	...
	initCommands := [][]byte{
		{0x55, 0xAA, 0x52, 0x05, 0x00, 0x00},
		{0x55, 0xAA, 0x52, 0x05, 0x00, 0x00},
		{0x55, 0xAA, 0x53, 0x05, 0x00, 0x00},
		{0x55, 0xAA, 0x53, 0x05, 0x00, 0x00},
		{0x55, 0xAA, 0x53, 0x05, 0x00, 0x00},
		{0x55, 0xAA, 0x40, 0x05, 0x00, 0x00},

		{0x55, 0xAA, 0x51, 0x09, 0x00, 0xA4, 0x90, 0x00, 0xFF, 0xFF},
		{0x55, 0xAA, 0x51, 0x09, 0x00, 0xA4, 0x90, 0x00, 0xFF, 0xFF},
		{0x55, 0xAA, 0x51, 0x09, 0x00, 0xA4, 0x90, 0x00, 0xFF, 0xFF},
		{0x55, 0xAA, 0x51, 0x09, 0x00, 0xA4, 0x90, 0x00, 0xFF, 0xFF},
		{0x55, 0xAA, 0x51, 0x09, 0x00, 0xA8, 0x00, 0x07, 0x00, 0x00},
		{0x55, 0xAA, 0x51, 0x09, 0x00, 0x18, 0xFF, 0x0F, 0xC1, 0x00},
		{0x55, 0xAA, 0x51, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x8B, 0x00},
		{0x55, 0xAA, 0x51, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x80, 0x18},

		// DEFINE TICKET MASK 255 and 16
		{0x55, 0xAA, 0x51, 0x09, 0x00, 0x14, 0x00, 0x00, 0x00, 0xFF},
		//{0x55, 0xAA, 0x51, 0x09, 0x00, 0x14, 0x00, 0x00, 0x00, 0xF1},

		{0x55, 0xAA, 0x51, 0x09, 0x00, 0x58, 0x02, 0x11, 0x11, 0x11},
		{0x55, 0xAA, 0x41, 0x09, 0x00, 0xA8, 0x00, 0x07, 0x00, 0x00},
		{0x55, 0xAA, 0x41, 0x09, 0x00, 0x18, 0xF0, 0x00, 0xC1, 0x00},

		{0x55, 0xAA, 0x41, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x8B, 0x00},
		{0x55, 0xAA, 0x41, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x80, 0x18},
		{0x55, 0xAA, 0x41, 0x09, 0x00, 0x3C, 0x80, 0x00, 0x82, 0xAA},

		{0x55, 0xAA, 0x51, 0x09, 0x00, 0x54, 0x00, 0x00, 0x00, 0x03},
	}
	util.AppLog("BITMAIN", fmt.Sprintf("Setting all frequency to %.2fMhz", 6.25*float64(frequencyTarget)), "")
	for freq := 10; freq <= frequencyTarget; freq++ { // 64 = 400mhz    48 = 300mhz   88=550mhz    104 = 650mhz
		initCommands = append(initCommands, calculatePLL(float64(6.25)*float64(freq), 96, 198))
	}
	initCommands = append(initCommands, []byte{0x55, 0xAA, 0x51, 0x09, 0x00, 0x10, 0x00, 0x00, 0x15, 0xa4})

	initCommands = append(initCommands, []byte{0x55, 0xAA, 0x51, 0x09, 0x00, 0xA4, 0x90, 0x00, 0xFF, 0xFF}) // TEST FROM BM1366 ESPMULTI
	if AonUserConfig.BaudRate == 1 {
		initCommands = append(initCommands, []byte{0x55, 0xAA, 0x51, 0x09, 0x00, 0x28, 0x11, 0x30, 0x02, 0x00}) //1M BAUD
	}
	if AonUserConfig.BaudRate == 2 {
		initCommands = append(initCommands, []byte{0x55, 0xAA, 0x51, 0x09, 0x00, 0x28, 0x11, 0x30, 0x01, 0x00}) // 1.5M BAUD
	}
...
func calculatePLL(targetFreq float64, fbDivMin, fbDivMax uint16) []uint8 {
	...
	return []uint8{0x55, 0xAA, 0x51, 0x09, 0x00, 0x08, vdoScale, params.FBDivider, params.RefDiv, postDiv}
}
